 ...
 * @since      1.0.0
 * @package    GutenBricks_Archive
 * @author     schrittweiter GmbH <info@schrittweiter.de>
 */
class GutenBricks_Archive {
	...
	private function define_admin_hooks(): void {
		...
		add_action(
			...
			function () {
				$this->enqueue_bud_entrypoint( 'gutenbricks-archive-admin' );
			},
			100
		);
	...
	private function define_public_hooks(): void {
		...
		add_action(
			'wp_enqueue_scripts',
			function () {
				$this->enqueue_bud_entrypoint( 'gutenbricks-archive-frontend' );
			},
			100
		);
	...
	private function enqueue_bud_entrypoint( string $entry, array $localize_data = array() ): void {
		$entrypoints_manifest = GUTENBRICKS_ARCHIVE_PATH . '/dist/entrypoints.json';
				...
				foreach ( $files as $file ) {
					if ( 'js' === $type ) {
						wp_enqueue_script(
							...
							GUTENBRICKS_ARCHIVE_URL . 'dist/' . $file,
