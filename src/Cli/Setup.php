...
namespace GutenBricks_Archive\Cli;
...
class Setup {
	...
	public function __invoke( array $args, array $assoc_args ): void { // phpcs:disable Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
		...
		if (
			...
			|| ! $this->replace_in_files( 'GutenBricks Archive', $this->name, array( '.*\.php', '.*README\.txt' ) )
		) {
	...
	private function rename_files(): void {
		...
		if (
			! rename( $this->path . '/src/GutenBricks_Archive.php', $this->path . "/src/$this->namespace.php" ) // phpcs:disable WordPress.WP.AlternativeFunctions.rename_rename
			|| ! rename( $this->path . '/gutenbricks-archive.php', $this->path . "/$this->slug.php" ) // phpcs:disable WordPress.WP.AlternativeFunctions.rename_rename
		) {
